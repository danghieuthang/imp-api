...
{
    public class GetAllCampaignMobileQuery : PageRequest, IListQuery<CampaignViewModel>
    {
        public GetAllCampaignMobileQuery()
        {
            Status = new List<int>();
        ...
        public class GetAllCampaignMobileQueryValidator : PageRequestValidator<GetAllCampaignMobileQuery, CampaignViewModel>
        {
        }

        public class GetAllCampaignMobileQueryHandler : ListQueryHandler<GetAllCampaignMobileQuery, CampaignViewModel>
        {
            private readonly IGenericRepository<Campaign> _campaignRepository;
            private readonly IAuthenticatedUserService _authenticatedUserService;
            public GetAllCampaignMobileQueryHandler(IUnitOfWork unitOfWork, IMapper mapper, IAuthenticatedUserService authenticatedUserService) : base(unitOfWork, mapper)
            {
                _campaignRepository = unitOfWork.Repository<Campaign>();
                _authenticatedUserService = authenticatedUserService;
            }
            public override async Task<Response<IPagedList<CampaignViewModel>>> Handle(GetAllCampaignMobileQuery request, CancellationToken cancellationToken)
            {
                ...
                var page = await _campaignRepository.GetPagedList(
                    predicate: x => (request.Status.Count == 0 || (request.Status.Count > 0 && request.Status.Contains(x.Status)))
                        ...
                        && x.LastModified != null
                        && !x.CampaignMembers.Any(y => y.InfluencerId == _authenticatedUserService.ApplicationUserId),
